...
export async function getEmployeeUtilization(
	...
	startDate?: Date,
	endDate?: Date
): Promise<EmployeeUtilizationData> {
	const { user } = await validateRequest();
	if (!user) {
		throw new Error("Unauthorized");
	}

	const taskWhere: Prisma.TaskAssignmentWhereInput = { employeeId };
	if (startDate || endDate) {
		taskWhere.startTime = {};
		if (startDate) taskWhere.startTime.gte = startDate;
		if (endDate) taskWhere.startTime.lte = endDate;
	}

	const taskAssignments = await db.taskAssignment.findMany({ where: taskWhere });

	const totalTaskHours = taskAssignments.reduce<number>(
		(total, assignment) => total + hoursBetween(assignment.startTime, assignment.endTime),
		0
	);

	const totalShiftHours = 0;

	return {
		employeeId,
		totalTaskHours,
		totalShiftHours,
		utilization: totalShiftHours > 0 ? totalTaskHours / totalShiftHours : 0,
	};
}
...
export async function generateExcelDownload(
	data: Array<Record<string, unknown>>,
	filename: string
): Promise<Response> {
