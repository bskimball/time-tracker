...
export function KPICard({
	...
	return (
		<Card className="relative overflow-hidden group bg-card">
			{/* Micro corner accents - status-aware hierarchy */}
			<div
				className={`corner-card-tl corner-accent-xs ${
					color === "green"
						? "corner-success"
						: color === "red"
							? "corner-destructive"
			...
			/>
			<div
				className={`corner-card-tr corner-accent-xs ${
					color === "green"
						? "corner-success"
						: color === "red"
							? "corner-destructive"
			...
			<div
				className={`absolute top-0 right-0 w-32 h-32 bg-linear-to-br ${styles.gradient} opacity-20 rounded-bl-full -mr-8 -mt-8 transition-opacity group-hover:opacity-30`}
			/>
							...
							<div
								className={`flex items-center gap-1 text-xs font-semibold px-2 py-1 rounded-full ${
									trend.direction === "up" && color === "green"
										...
										: trend.direction === "down" && color === "red"
											? "bg-rose-100 text-rose-700 dark:bg-rose-500/20 dark:text-rose-400"
											: trend.direction === "up" && color === "red"
												? "bg-rose-100 text-rose-700 dark:bg-rose-500/20 dark:text-rose-400" // Bad up
												: trend.direction === "down" && color === "green"
													? "bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400" // Good down
