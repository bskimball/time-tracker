...
```typescript
// apps/web/src/routes/manager/dashboard/route.tsx
...
```typescript
// apps/web/src/routes/employees/actions.ts
"use server";
...
```typescript
// apps/web/src/routes/api/employees.ts
...
const jobLogger = createLogger({ component: "background-job" });

export async function runBackgroundJob() {
	jobLogger.info("Starting background job");
...
**Configured in `apps/web/src/lib/logger.ts`:**
...
1. Add field to redaction list in `apps/web/src/lib/logger.ts`:
...
- `apps/web/src/lib/logger.ts` - Core logger configuration
- `apps/web/src/lib/request-context.ts` - Request-scoped logger
- `apps/web/src/lib/logging-helpers.ts` - Convenience utilities
- `apps/web/src/lib/db.ts` - Database query logging
