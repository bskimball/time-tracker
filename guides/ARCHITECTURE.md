...
│   │   └── performance-scheduler.ts # Background metrics
│   ├── routes/
...
Routes are defined in `src/routes/config.ts` using React Router's configuration format with custom middleware support.
...
Defined in `src/lib/middleware.ts`:
...
- **`guides/RSC_AUTH_FIX.md`** - Authentication patterns in RSC
