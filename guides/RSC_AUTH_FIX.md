# RSC Data Mode Authentication Fix
...
1. **Created Request Context** (`src/lib/request-context.ts`):
...
2. **Updated Auth Library** (`src/lib/auth.ts`):
...
4. **Updated Route Components**:
   - Removed `{ request }: { request: Request }` props from:
     ...
     - `src/routes/time-clock/route.tsx`
     - `src/routes/todo/route.tsx`
