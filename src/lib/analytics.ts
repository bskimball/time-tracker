...
export async function getExecutiveKPIs(startDate: Date, endDate: Date): Promise<PerformanceKPIs> {
	...
	const performanceData = await prisma.performanceMetric.aggregate({
		where: {
			date: {
...
export async function getStationPerformance(
	...
	const stationPerformance = await prisma.performanceMetric.groupBy({
		by: ["stationId"],
		where: {
			date: {
				gte: startOfDay(startDate),
...
async function getCurrentOccupancy(): Promise<number> {
	const activeSessions = await prisma.timeLog.count({
		where: {
			endTime: null,
			type: "WORK",
		},
	...
	const totalCapacity = await prisma.station.aggregate({
		where: { isActive: true },
		_sum: { capacity: true },
	});
...
export async function getPerformanceTrends(
	...
	for (let i = 0; i < days; i++) {
		...
		const dayMetrics = await prisma.performanceMetric.aggregate({
			where: {
				date: {
					gte: startOfDay(date),
					lte: dayEnd,
